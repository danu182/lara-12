...
{{-- <div class="container-fluid"> --}}
    ...
    <p class="text-l font-weight-bold text-info mb-4">{BASE URL}/{Service Name}/{{ $alamat }}</p>
    ...
    <div class="row">
        @if( $data['metaData']['code']=='500' || $data['metaData']['code']<>'200' )
            ...
            {{-- <div class="alert alert-danger">
                <p>"code" = {{ $data['metaData']['code'] }}</p>
                <p>"message" = {{ $data['metaData']['message'] }}</p>
                <p>"Parameter1" = {{ $Parameter1 }}</p>
                <p>"Parameter2" = {{ $Parameter2 }}</p>
            </div> --}}
        @else
            @forelse ( $data['response']['poli'] as $key => $val )
                <div class="col-lg-6">
                    <div class="card mb-4 py-3 border-left-primary">
                        <div class="card-body">
                            <h6 class="m-0 font-weight-bold text-primary">{{ ucfirst($key+1) }}</h6>
                            @if (is_array($val))
